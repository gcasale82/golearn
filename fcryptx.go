...
func removeNotFile(files []string) []string {
for i , f := range files {
...
fmt.Println("cleaning password from memory...")
ptr := unsafe.Pointer(&password)
	size := uintptr(len(password))
